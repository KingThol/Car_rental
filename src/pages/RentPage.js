...
function Rent() {
  ...
  // Mengambil data mobil saat pertama kali komponen dimuat
  useEffect(() => {
    axios
  ...
  // Fungsi untuk menghitung harga berdasarkan tanggal sewa dan pengembalian
  const calculatePrice = () => {
    ...
    // Validasi tanggal
    if (isNaN(rental_date) || isNaN(return_date)) {
      setTotalPrice(0);
      return;
    }
  ...
  // Menangani perubahan tanggal sewa dan pengembalian
  ...
  // Jika data mobil sedang dimuat, tampilkan spinner
  ...
  const rentACar = (e) => {
    ...
    // Validasi input
    if (isNaN(rental_date) || isNaN(return_date)) {
      showToast(toast, "Please select valid rental and return dates.", "error", "Error");
      return;
    }

    if (rental_date < now || return_date < now) {
      showToast(toast, "Please select future rental and return dates.", "error", "Error");
      return;
    } else if (rentDuration <= 0) {
      showToast(toast, "Rental duration must be greater than 0.", "error", "Error");
      return;
    }
    ...
    // Jika semua input sudah valid, kirim data sewa ke API
      ...
      <Center h={"100vh"} m={["5%", "10%", "12%", "13%", "0%"]}>
        ...
        >
          <Box w={{ base: "100%", lg: "50%" }}>
            <Image src={car.photo2} objectFit="cover" h={"full"} />
          </Box>
